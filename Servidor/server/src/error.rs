...
pub enum AppError {
    ...
    // --- 404 Not Found (codes) ---
    #[error("prepaid code not found")]
    CodeNotFound,

    // --- 409 Conflict ---
    #[error("prepaid code is exhausted, expired, or invalid")]
    InvalidPrepaidCode,

    #[error("prepaid code has no remaining jobs")]
    CodeExhausted,
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        let (status, message) = match &self {
            ...
            AppError::CodeNotFound => (StatusCode::NOT_FOUND, self.to_string()),

            // 409
            AppError::InvalidPrepaidCode => (StatusCode::CONFLICT, self.to_string()),
            AppError::CodeExhausted => (StatusCode::CONFLICT, self.to_string()),
