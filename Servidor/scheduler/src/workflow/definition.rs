...
pub fn step_for_state(
    ...
    match &job.state {
        ...
        JobState::ResultsGenerating => {
            // Results container has finished — job is complete.
            None
        }
...
fn post_mintpy_step(
    ...
    match workflow {
        ...
        _ => {
            // SBAS and time-series workflows generate results after MintPy.
            Some(results_step(job_workdir, workflow_cfg, images))
        }
    }
}
...
/// Launches the results container to generate output products (velocity.png)
/// after MintPy completes for SBAS and time-series workflows.
fn results_step(
    job_workdir: &str,
    workflow_cfg: &WorkflowConfig,
    images: &ContainerImages,
) -> WorkflowStep {
    let resources = ResourceRequirement::from(&workflow_cfg.tiers.light);

    WorkflowStep {
        label: "results.velocity".to_string(),
        next_state: JobState::ResultsGenerating,
        spec: ContainerSpec {
            image: images.results.clone(),
            args: vec!["velocity".to_string()],
            host_archive: job_workdir.to_string(),
            host_workdir: job_workdir.to_string(),
            container_workdir: CONTAINER_WORKDIR.to_string(),
            resources,
...
// =============================================================================
// Result request container spec
// =============================================================================

/// Builds the `ContainerSpec` for a result request.
///
/// Returns `None` for result types that are not yet implemented (stubs).
///
/// The `output_path` is the **absolute host path** where the container must
/// write its output file (e.g. `{work_dir}/results/velocity.png`).  It is
/// passed as the `--output` argument to the `sisar-results` binary inside the
/// container.
pub fn result_container_spec(
    result_type: &shared::models::ResultType,
    params: &Option<serde_json::Value>,
    job_work_dir: &str,
    output_path: &str,
    image: &str,
) -> Option<ContainerSpec> {
    use shared::models::ResultType;

    let args: Vec<String> = match result_type {
        ResultType::Velocity => vec![
            "velocity".to_string(),
            "--output".to_string(),
            // Inside the container the workdir is mounted at /job, so we
            // translate the host output path to the container path.
            host_to_container_path(output_path, job_work_dir),
        ],

        ResultType::Timeseries => {
            let lat = params.as_ref()?.get("lat")?.as_f64()?;
            let lon = params.as_ref()?.get("lon")?.as_f64()?;
            // FIX: Use --flag=VALUE syntax instead of --flag VALUE.
            // When lat or lon is negative (e.g. -38.13), passing it as a
            // separate token makes clap interpret it as an unknown flag.
            // The = form passes the value as part of the same token,
            // so clap never mistakes it for a flag.
            vec![
        ...
        // Not yet implemented — caller should reject these before reaching here.
        ResultType::AiSummary | ResultType::Dem | ResultType::Model3d => return None,
    };

    Some(ContainerSpec {
        image: image.to_string(),
         args,
         host_archive: job_work_dir.to_string(),
         host_workdir: job_work_dir.to_string(),
         container_workdir: CONTAINER_WORKDIR.to_string(),
         // Results container is lightweight — no resource constraints.
         resources: ResourceRequirement {
             cpu_cores: 0,
             ram_gb:    0.0,
         },
...
/// Translates an absolute host path that lives under `job_work_dir` into the
/// equivalent path inside the container (`/job/...`).
fn host_to_container_path(host_path: &str, job_work_dir: &str) -> String {
    // Strip the host work_dir prefix and prepend the container mount point.
    let relative = host_path
    .strip_prefix(job_work_dir)
    .unwrap_or(host_path)
    .trim_start_matches('/');
    format!("{CONTAINER_WORKDIR}/{relative}")
}
